...
void CHustBaseApp::OnCreateDB()
{
	...
	if(lpDlist!=NULL)
	{
		SHGetPathFromIDList(lpDlist,str.GetBuffer(MAX_PATH)); //取出路径
		str.ReleaseBuffer();
		dbPath=str.GetBuffer(0);
		dbName=szPath;
		tmp=CreateDB(dbPath,dbName);
		if(tmp!=SUCCESS)
		{
...
void CHustBaseApp::OnOpenDB() 
{
	...
	CFileFind fileFind;
	BOOL table_Exist = (BOOL)fileFind.FindFile("SYSTABLES.xx");
	BOOL columns_Exist = (BOOL)fileFind.FindFile("SYSCOLUMNS.xx");
	if (!table_Exist||!columns_Exist)
	{
		AfxMessageBox("未找到系统文件！");
		return;
	}
	OpenDB(dbPath);
